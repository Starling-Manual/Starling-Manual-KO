...
노트: 계속하기 전에 '커스텀 필터, Custom Filters' 섹션을 읽었는지 확인하십시오. 필터와 스타일은 많은 개념을 공유하므로 두 가지 중 더 간단한 것으로 시작하는 것이 좋습니다. 아래에서는 다른 섹션에 표시된 모든 내용을 잘 알고 있다고 가정합니다.
...
중요: 그것이 중요하기 때문에 나는 다음과 같이 다시 한번 말합니다:
...
노트: 이 두 클래스는 TexturedEffect와 ColoredTexturedEffect라는 이름이 붙어있을 수도 있지만 사용법을 염두에두고 이름을 지정했습니다. 필터를 만들면 FilterEffect를 확장해야 합니다. 메쉬 스타일, MeshEffect를 만든다면.
